...
5. `check_encoding_lint()` — runs `tools/encoding_lint.py --check`; flags subprocess.run/check_output/Popen calls with text=True/universal_newlines=True without encoding= (Guardrail G10 extension). Detects Windows cp1252 encoding surprises. Fail-open only when tool missing; fail-closed on findings.
6. `check_test_coverage()` — runs `tools/verify_test_coverage.py --check`; detects orphaned test files not run by any CI job (Guardrail G2 extension). Prevents "fake-green" test suites. Fail-open only when tool missing; fail-closed on findings.
