...
## Dropped (reason)
- Python count revised 163→164 (adds test_wave_e2e_first_wave [7 new: first-wave e2e proof — CLI invocation runs minimal 2-3 item wave against fixture repo in tmpdir, FakeDriver fixes items to green via test exit 0, captures JSON wave report, asserts on shape + final state hash, fixture isolation (no cwd/git-config pollution), saves sample report to tests/fixtures/first-wave-report.json]).
