...
## Dropped (reason)
- Python count revised 175→183 (WS4 increment 1 adds test_gen_state_md [8 new: STATE.md generator from state store via read facade (StateAPI), empty store, tracker item rendering by lane, output to file (--out flag), stdout default, deterministic output, ASCII-safe, malformed store exit 1, CLI integration]).
