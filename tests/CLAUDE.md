...
Actual suite counts are GENERATED and tracked in `tests/SUITE-COUNTS.json` (regenerated by `tools/gen_suite_counts.py --regenerate` when tests are added/removed). See that file for the current counts.

**Shell**: Discovered dynamically from `tests/*.test.sh`, `tests/test_*.sh`, `tests/test-*.sh` plus `hooks/pre-push-policy.sh --test`.
...
**Counts are generated, not hand-maintained**: `python tools/gen_suite_counts.py --check` is READ-ONLY — it fails the pre-push hook and CI when the counts in `tests/SUITE-COUNTS.json` drift from actual test files. When your lane adds or removes a test suite, run `python tools/gen_suite_counts.py --regenerate` and commit the updated `tests/SUITE-COUNTS.json` in the same PR. This eliminates the merge-conflict serialization that occurred when multiple PRs edited hand-maintained count lines in this file.
