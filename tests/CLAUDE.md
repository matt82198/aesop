...
## Dropped (reason)
- Python count revised 134→135 (RS3-W adds test_wave_loop_rs3 [24 new: N1 flagship Claude-worker+swapped-seat wave ships without TypeError + seat-spend re-check, N5 claim exception fail-closed skip, N4 stale-lease release key parity + coordination TTL expiry + EventStore read compat, N3 resumed-verified filesWritten restore/ship/no_changes terminal record + scheduler tracker-terminal no-reselect, N6 Windows forced-quote backslash rules + git add execution proof, N7 executor-failure recording + green-not-vacuous + duplicate-slug preflight, N10 fingerprint rebuild/resume/legacy + torn journal + atomic replace]).
