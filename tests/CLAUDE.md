...
**Shell (12 suites)**:
backup-fleet.test.sh, dash-watchdog-gui.test.sh, test_agent_forensics.sh, test_pre_push_policy.sh, test-run-watchdog.sh, test-run-watchdog-halt.sh, test-run-watchdog-lockguard.sh, test-run-watchdog-smoke-signal.sh, test_reconstitute.sh, test_reconstitute_fixes.sh, test-selfheal.sh, test_waveguard.sh.
...
## Dropped (reason)
- Python count revised 175→176 (WS4 increment 2 adds test_ws4_writer_migrations [10 new: top-3 writer migration to WriteAPI facade — buildlog.py appends land as buildlog_entry events + legacy header preserved once, ensure_state.py scaffold emits state_md_written + buildlog events + idempotent rerun appends none, eod_sweep.py verdicts land as events + timestamp format preserved + non-canonical --buildlog filename refused fail-closed; .stateapi-baseline.json ratcheted 42→37]).
- Python count revised 177→178 (G6 guardrail adds test_subprocess_guard [14 new: bare bash-without-cwd, shell=True, os.system, explicit cwd=None, cwd= passes clean, `# subprocess-ok` suppression scoped to the marked call only, JSON output shape, CLI exit codes + --help hygiene]).
- Python count revised to 167 (merge of feat/first-wave-e2e with origin/main: HEAD adds test_wave_e2e_first_wave [7 new: first-wave e2e proof — CLI invocation runs minimal 2-3 item wave against fixture repo in tmpdir, FakeDriver fixes items to green via test exit 0, captures JSON wave report, asserts on shape + final state hash, fixture isolation (no cwd/git-config pollution), saves sample report to tests/fixtures/first-wave-report.json]; origin/main merges test_agent_prompt_hygiene [Security audit] + test_traps [14 new: adversarial regression trap suite encoding 5 incident classes from docs/INCIDENTS.md — fake-green (3), gate-activation (3), doc-invented (3), test-pollution (2), ci-drift (2), plus 1 test documenting non-mechanizable classes (conflict, flake, stall); each trap cites source incident and fails if pattern re-emerges; reuses/extends test_test_hygiene.py and verify_test_suite_count.py gates]).
- Python count revised 166→177 (WS4 increment 1 adds test_writeapi_markdown [11 new: WriteAPI markdown write-path unification (STATE.md, BUILDLOG.md); atomic event + file appends, OCC conflict detection, round-trip verification, PR #499]).
- Python count revised 163→164 (adds test_traps.py [14 new: adversarial regression trap suite encoding 5 incident classes from docs/INCIDENTS.md — fake-green (3), gate-activation (3), doc-invented (3), test-pollution (2), ci-drift (2), plus 1 test documenting non-mechanizable classes (conflict, flake, stall); each trap cites source incident and fails if pattern re-emerges; reuses/extends test_test_hygiene.py and verify_test_suite_count.py gates]).
- Python count revised 164→165 (Guardrail G2 adds test_verify_test_coverage [8 new: orphan detection for Python/Node/Shell/Playwright test files, --check exit codes, --fix suggestions, path normalization for Windows]).
