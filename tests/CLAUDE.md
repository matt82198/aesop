...
## Dropped (reason)
- Python count revised 164→165 (Guardrail G3 adds test_watcher_linter [15 new: while-true-sleep AST detection, watcher-named (watch_/monitor_/poll_) infinite-loop detection, subprocess-in-infinite-loop detection, exit-path exemption (break/return/raise/exit) so legitimate bounded poll-until-timeout loops like ci_merge_wait.py are not flagged, dispatch-prompt wait/poll/watch phrasing detection scoped to prompt-ish assignment/kwarg/dict-key context (not arbitrary comments/prints), # watcher-ok suppression, --json CLI output, --paths override, self-exclusion of the linter's own file from directory scans]).
