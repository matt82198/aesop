...
class MergeQueueEncodingTest(unittest.TestCase):
    """Test merge-queue daemon encoding handling for non-UTF-8 bytes."""

    def test_gh_function_handles_em_dash_in_output(self):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            self.assertTrue("em" in result or isinstance(result, dict))

    def test_git_function_handles_em_dash_in_output(self):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            self.assertTrue(ok)
            self.assertTrue("commit" in out or len(out) > 0)

    def test_classify_check_handles_em_dash(self):
        ...
        self.assertTrue("em" in name or "Check" in name or "—" in name)

    def test_required_checks_green_with_em_dash(self):
    ...
    def test_subprocess_with_actual_em_dash_byte(self):
