...
def test_counter_example_rejects():
...
def test_discrimination_rationale_present():
...
if __name__ == "__main__":
    # Run all tests
    try:
        test_all_tasks_loaded()
        test_task_ids_sequential()
        test_patterns_are_valid()
        test_exemplar_matches()
        test_counter_example_rejects()
        test_discrimination_rationale_present()
        test_no_duplicate_categories()

        print("\n" + "="*70)
        print("✓ ALL TESTS PASSED: Frontier slice N=60 is valid and ready for v2 run")
        print("="*70)
        sys.exit(0)
