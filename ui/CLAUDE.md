...
**tooling_panel.py** â€” Tooling dashboard panel: `GET /api/tooling/summary` aggregates results from repo analysis tools (todo_tracker, test_coverage_gaps, dead_code_check, import_cycle_check, encoding_lint). Runs tools via subprocess (30s timeout), caches 60s, gracefully degrades to null for missing tools. `?force=1` bypasses cache. **Exit code handling (findings-aware linters):** Some tools signal “findings detected” with exit code 1, not an error — `_is_acceptable_exit_code()` distinguishes: dead_code_check.py, import_cycle_check.py, encoding_lint.py exit with 0 (clean) or 1 (findings found, both valid) or 2+ (error); other tools only accept 0. This allows accurate reporting of findings without masking them as errors.
...
**New:** `ui/web/src/components/CostSummaryDrawer.tsx` — right-edge collapsible drawer (40px collapsed, ~180px expanded) showing spend/rate/model-mix metrics. Bound to SSE `cost`+`connectionStatus` props; handles loading/error/empty/loaded states. Toggle button + panel with `role="status"`, `aria-label` accessibility. 31 unit tests (vitest, all states + keyboard/pricing + stale-after-disconnect + model-mix proportions). Integrated into App.tsx. CSS uses design tokens. Cost.tsx:63 comment fixed (was "live/reconnecting", now "live"). **Contract fixes (drawer P1s):** connection status wins over cached data — `useSSE` retains the last `cost` when the EventSource errors, so anything gated on `!cost` is unreachable after the first payload and the always-visible drawer showed stale spend indefinitely; `isError` is now `connectionStatus.status !== 'live'` ALONE, precedence error (retained payload rendered under an explicit `cost-summary-stale` label, never as current) > loading > empty > loaded. Model-mix shares are each model's fraction of the **all-time** token total, both sides from `summary.models` (`daily_totals` has no model dimension) — same window, so shares sum to 100%, the window is labelled in the UI, and the 0.1% rounding happens once so label and bar width cannot disagree (the old denominator mixed windows, exceeded 100%, saturated every bar). `tools/verify_cost_summary_drawer.py` carries a falsifiability contract: every docstring claim is an `assert` and each `[PASS]` print sits after the assert earning it — an unassertable claim is deleted from the docstring, never downgraded to `[WARN]`/`[INFO]`.
**CostForecast addition:** `ui/web/src/components/CostForecast.tsx` (+ .test.tsx + .css) — linear regression trend over daily_totals; end-of-wave projection + <!-- metrics-verified: residual-std-multipliers-1.04-1.64-confidence-70-90 --> 70%/90% confidence bands (residual-based, labeled honestly); ceiling alerts. 24 unit tests (empty/populated/ceiling states). Wired into CostAnalyticsPanel. tools/verify_cost_forecast.py browser proof (5-day ledger + empty state).