...
- **run-watchdog.sh**: Daemon supervisor (1.8K); spawns backup-fleet.sh every 150s with atomic lockfile guard, maintains heartbeat, logs to FLEET-BACKUP.log, posts security alerts via alert_bridge.py (opt-in). Traps INT/TERM cleanly. **Halt detection (wave-27 safety fix)**: check_halt() calls `python tools/halt.py --status` as the single source of truth, respecting AESOP_STATE_ROOT env var > aesop.config.json state_root > default; bash never re-derives the sentinel path, eliminating drift from configuration overrides. **BASH_SOURCE exec-guard**: all path/env variables declared INSIDE the guard (sourcing exposes only functions like acquire_lock/release_lock/check_halt, not env side effects).
...
- **run-merge-queue.sh**: Merge-queue advancer runner (~130 lines). NOT a daemon and NOT a loop: one invocation runs exactly ONE bounded pass of `tools/merge_queue.py --advance`, and the 5-minute `AesopMergeQueue` Scheduled Task IS the loop. Deliberate -- merging must not depend on a live interactive session (measured green->merge dead time ~31.75 HOURS sessionless vs 9-109 seconds seated). Zero `sleep` calls, zero polling. **Halt detection (wave-27 safety fix)**: check_halt() calls `python tools/halt.py --status` as the single source of truth, respecting AESOP_STATE_ROOT env var > aesop.config.json state_root > default; bash never re-derives the sentinel path, eliminating drift from configuration overrides. `cd`s to `$AESOP_ROOT` because `gh` resolves the repo from cwd, exports `AESOP_STATE_ROOT`, appends to state/MERGE-QUEUE.log, and propagates the tool's exit code. **BASH_SOURCE exec-guard**: every path/env variable declared INSIDE the guard, so sourcing exposes only resolve_python/log_line/check_halt.
...
- `.HALT`: Kill-switch sentinel (JSON {reason}); checked at cycle start by run-watchdog.sh and run-merge-queue.sh via `python tools/halt.py --status` as the single source of truth. Daemon reads halt.py's resolution chain (AESOP_STATE_ROOT env > aesop.config.json state_root > `$AESOP_ROOT/state/.HALT`), so halt.py --status and daemon are always synchronized. A halted cycle logs "HALTED: <reason>" and skips all work until cleared. Behavioral proof test (test-daemon-halt-config-path.sh) covers both AESOP_STATE_ROOT and config overrides. Prior drift (daemon re-deriving sentinel path) fixed in wave-27.
...
bash tests/test-daemon-halt-config-path.sh
