...
- **write_api.py** — `WriteAPI(state_dir)` facade for tracker mutations (state consolidation write path). Exposes tracker CRUD and markdown operations: `tracker_append_item(item_dict)`, `tracker_update_status(item_id, new_status, note)`, `tracker_update_item(item_id, update_data)` (general-purpose patch), `tracker_archive_item(item_id)` (soft-delete), and `rebuild_projection()`. 
  ...
  - **Snapshot optimization (2026-08-03)**: `_project_tracker()` now uses `project_tracker_with_snapshot()` (O(n) tail-replay) instead of `project_tracker()` (O(n²) full replay). Reduces per-mutation projection cost; gracefully falls back to full replay when snapshot missing/corrupt. OCC WriteConflict guard unchanged and load-bearing.
...
- **identity.py** — Multi-instance identity: `get_instance_id()` returns ephemeral form (hostname:pid:nonce); `get_identity_with_epoch(state_root)` returns durable (stable_id, epoch) persisted to $AESOP_STATE_ROOT/instance-id. Epoch is the monotonic boot counter used as the multibox fencing token. **Acquisition increments it** (atomic read-modify-write: temp file + fsync + `os.replace`), so every boot on a state root gets a strictly higher epoch than every prior boot; the value is cached per process, so repeated calls within one process return one epoch. **Fail-closed on corruption**: a corrupt existing file — torn/partial JSON, empty, missing keys, or a non-integer/`<1` epoch that cannot be incremented monotonically — raises `IdentityCorruptionError`. **Fail-closed on persist failure**: if the bumped epoch cannot be written, raises `EpochPersistError` (subclass of `IdentityCorruptionError`, so existing handlers still catch it) instead of handing back a reused epoch. **Fail-open on creation only**: fresh box (no prior file) with an unwritable directory falls back to ephemeral (allows solo mode). Distinction preserves monotonicity: a stale crashed instance cannot claim epoch=1 after restart. `release_own_stale()` raises `NotImplementedError` — prior-epoch claim reclamation needs the Inc 5 lease-backend coordination; it previously returned an unconditional `True`, so a caller believed reclamation had happened when nothing ran. Until Inc 5, prior claims are reclaimed by TTL expiry.
- **paths.py** — `canonical_claim_path(path, repo_root=None, case_policy=...)`: THE claim-key canonicalizer, and the only place claim-path normalization logic lives. Folds separators to `/` via `posixpath` (never `os.path`, which on Linux treats `\` as an ordinary filename character and would leave a Windows-style path uncollapsed), collapses `.`/`..`, NFC-normalizes Unicode, then case-folds per `case_policy`: `"platform"` follows `os.name`, `"insensitive"`/`"sensitive"` are host-independent. Idempotent.
- **coordination.py** — Lease-by-append claims for multi-writer coordination: `try_claim(store, resource, instance_id, ttl)` / `release` / `current_holder` / `fold_claims` / `compact_claims` via fail-closed event appends. Accepts a StateAPI (`.get`) OR a raw EventStore (`.read`) — RS3-W fix: try_claim previously required `.get()` so every EventStore claim fail-closed to False (dead gate). TTL expiry is ENFORCED at fold time: a claim past `ts + ttl` is ignored/reclaimable (crashed holders cannot hold forever; legacy events without ts/ttl never expire). RS5 F2: a try_claim whose post-append read/fold raises retracts its own claim_requested (best-effort) before returning False — a failed claim attempt never strands a phantom holder for a full TTL. Claims-stream compaction: `compact_claims(store)` snapshots the active claim events for O(tail) reads instead of O(total). Prevents concurrent orchestrators from colliding. **Snapshot optimization (2026-08-03)**: `try_claim()` now uses `_read_claims_compacted()` (snapshot + tail-replay, O(n)) instead of `_read_claim_events()` (O(n²) full scan on every parallel dispatch). Enables fleet to scale to larger claim histories without mutation cost explosion.
- **lease_claims.py** — `LeaseStore(db_path, case_policy=None, config=None)`: atomic file-path leasing for multi-instance coordination using a dedicated SQLite leases table (not event-sourced, for atomicity + TTL efficiency). API: `claim(paths, instance_id, ttl_seconds)→lease_id` (atomically claims set of paths; raises `LeaseConflict` if any held by another active instance), `renew(lease_id, instance_id, ttl_seconds)` (extends TTL; fails if not holder), `release(lease_id, instance_id)` (releases lease; frees paths for others), `get_holder(paths)`. Deterministic time injection (clock parameter) enables testing without sleeps. Expiry-steal-on-claim: leases past deadline are reclaimable immediately at claim time (no background thread). All operations fail-closed: exceptions propagate.
  - **Claim keys are host-independent.** `_normalize_path(path, case_policy=None)` is the SINGLE normalization entry point used for both storage and conflict checking, and delegates to `paths.canonical_claim_path`. It must never infer the policy from `os.name`: with `case_policy="platform"` the same file keyed as `tools/runner.py` on Windows and `tools/Runner.py` on Linux, so `_check_conflicts` (exact match) missed and BOTH instances were granted the claim — split-brain. Any guard test proving host-independence must call `_normalize_path` (calling `canonical_claim_path` directly with a policy production never passes proves nothing).
  - **Case policy resolution** (`resolve_case_policy(config)`): explicit `case_policy=` arg → `config["multibox"]["case_policy"]` → `$AESOP_CLAIM_CASE_POLICY` → `DEFAULT_CASE_POLICY` (`"insensitive"`). An unrecognized policy raises `ValueError` (fail-closed) — a silent fallback would change the keyspace unnoticed, the same bug class again. When no explicit policy or config is given, `LeaseStore` defers resolution to call time so env changes apply without restart.
  - **Why `"insensitive"` is the default**: it is the only policy that is identical on every host (what multibox requires), and it errs toward OVER-collision. A claim keyspace may safely collide two distinct files (the loser waits); it may never fail to collide two names for the SAME file (both instances write it). Case-sensitive single-box semantics remain available via `LeaseStore(db, case_policy="sensitive")` or `multibox.case_policy`.
- **claim_backend.py** — `ClaimBackend` protocol + `LocalLeaseBackend(db_path, config=None)` adapter over `LeaseStore`; `get_backend(db_path, config)` returns the backend when `multibox.enabled=True`, else `None` (legacy advisory path). Config is passed through to `LeaseStore` so `multibox.case_policy` reaches the claim keyspace.
