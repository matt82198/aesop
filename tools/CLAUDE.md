...
- `workflow_model_linter.py` — Guardrail G7: workflow model pin linter + workflow-args validator (esc-wf-args-string); AST-scans .js/.mjs files for (1) agent() calls missing explicit model:'haiku' parameter (bypasses PreToolUse hook), and (2) unsafe JSON.parse patterns (try-catch with silent empty-object fallback, unvalidated field access); detects root cause from wave-flat-dispatch.template.mjs (lines 92-93 escape); suppress via `// model-ok` (for model violations) or `// args-ok` (for parse violations); CLI: `--check` (default) | `--json` | `--help`; exit 0=clean/1=violations/2=error; stdlib-only
