...
- `orchestrator_context_meter.py` — Measures orchestrator context bloat (checkpoint age, activity since checkpoint, tokens); emits OK/ADVISE-CHECKPOINT/ADVISE-CLEAR verdict based on heuristic thresholds; read-only; CLI: `[--check] [--json] [--state-root DIR] [--checkpoint-age-hours H] [--activity-checkpoint-threshold N] [--activity-clear-threshold N]`; exit 0=OK/1=advisory/2=error
