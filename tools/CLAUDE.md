...
- `ci_gate_runability.py` — CI gate-runability validator (Guardrail G2.5): prevents "green can mean never ran" incidents by verifying known suite families (pytest, npm test:node, run_shell_tests.sh, playwright, verify_*.py, lint/guard gates) are not silently skipped due to branch protection misconfiguration; checks job/step-level if conditions that exclude PRs, continue-on-error on gates, missing file references; CLI: `[--check] [--json] [--root DIR]`; exit 0=clean/1=findings/2=error; stdlib-only; staged (wire into ci.yml after #596)
