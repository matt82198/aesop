...
- `lane_scheduler.py` — WS3a pilot: overlap-aware lane scheduling (file-disjoint saturation model). Schedules items into parallel "lanes" such that no two items in a lane overlap file ownership. API: `schedule_disjoint_lanes(items, max_lanes)` → Report with lanes[{lane_id, items}], unscheduled, invalid_items. CLI: `lane_scheduler.py --dry-run <manifest.json> [--max-lanes N]`. Deterministic output, platform-independent path normalization (fnmatch globbing for overlap detection), stdlib-only.
