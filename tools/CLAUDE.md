...
- `ci_shard_runner.py` — Shard-aware Python test runner (distributes tracked test files across N shards round-robin; spawn-safe with __main__ guard; PYTEST_TIMEOUT env var switches to pytest subprocess with per-test timeout via pytest-timeout; used by ci and windows-shard jobs)
