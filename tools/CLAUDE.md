...
- `regression_gate_validator.py` — Guardrail G8: enforces the rule "verify the exact gate, not a proxy" (esc-regression-lens-wrong-gate); detects audit/verification/regression dispatches citing pytest tests/ (development proxy) instead of tools/ci_shard_runner.py (actual CI gate); flags timeout exit code 124 in conditional logic that infers test failures; suppression via `# regression-gate-ok` inline comment; only scans files with regression/verification/audit/test context keywords; CLI: `--check` (default) | `--json`; exit 0=clean/1=violations/2=error; stdlib-only, ASCII output; false-positive filter rejects string constants (e.g., _TIMEOUT_NOTE), documentation strings, and requires actual code comparison operators (if/elif/while/==/!=) to flag timeout patterns
