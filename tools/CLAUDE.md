...
- `verify_test_suite_count.py` — Test suite count drift gate (single source of truth: 254 test suites = 25 Node + 13 Shell + 216 Python); fail-closed when git unavailable; CLI: `--check` (fail if counts drift; pre-push gate only -- NOT run by any CI workflow) | `--fix [--dry-run]` (auto-rewrite tests/CLAUDE.md counts to match disk; lanes use this); --repo/--claudemd overrides; idempotent; exit 0=clean/fixed, 1=drift, 2=error (fail-closed: git unavailable, subprocess failure); wired into hooks/pre-push-policy.sh via check_test_suite_count() to catch drift locally before push
