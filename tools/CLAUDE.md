...
- `tracker_autoclose.py` — Tracker zombie-prevention auto-close gate: classifies active items as SHIPPED (merged PR or ownsFiles on origin/main), OPEN (no evidence), or AMBIGUOUS (partial evidence); CLI: `[--check | --apply] [--json] [--skip-gh] [--skip-git]`; --check (DRY RUN, exit 0 if no closable items / 1 if closable found); --apply (auto-close + journal); exit 2 on error; reproduces/fixes 79% zombie-rate escapes (items shipped but in-progress)
...
- `verify_test_suite_count.py` — Test suite count drift gate (auto-verifiable + auto-fixable); CLI: `--check` (fail if counts drift; CI + pre-push gate) | `--fix [--dry-run]` (auto-rewrite tests/CLAUDE.md counts to match disk; lanes use this); --repo/--claudemd overrides; idempotent; exit 0=clean/fixed, 1=drift/error; wired into hooks/pre-push-policy.sh via check_test_suite_count() to catch drift locally before push
