...
## Baseline ratchet updates

- **2026-07-30 Inc 1.5 (stateapi-allowlist-materialize)**: Added `state_store/materialize.py` to `WRITER_ALLOWLIST` (canonical materializer for tracker.json, orchestrator-status.json, STATE.md; the only renderer of derived views). Baseline entries: 43 → 42 (removed 4 stale entries from healthcheck/orchestrator_status; added 3 state_rebuild entries as deferred debt). Regression test added: `test_baseline_entry_count_never_exceeds_committed_value` enforces SHRINK-ONLY ratchet property.
