...
- `verify_test_suite_count.py` — Test suite count drift gate (auto-verifiable + auto-fixable); CLI: `--check` (fail if counts drift) | `--fix [--dry-run]` (auto-rewrite tests/CLAUDE.md counts to match disk); --repo/--claudemd overrides; idempotent; exit 0=clean/fixed, 1=drift/error; **wired into pre-push hook** (esc-g5-595: catch test additions without documentation updates before push)
