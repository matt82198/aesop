...
- `mcp_tool_inventory_lint.py` — MCP tool inventory guardrail (Guardrail: mcp-tool-inventory-lint): verifies mcp/server.mjs tool definitions stay in sync with tests/mcp-fleet.test.mjs line 223 expectations via regex extraction and alphabetical comparison; CLI: `[--root AESOP_ROOT] [--json]`; exit 0=in-sync/1=mismatch/2=error; stdlib-only, hermetic. STAGED FOR CI WIRING: add to .github/workflows/ci.yml under 'ci' job after 'Node syntax check' on shard 0 only (~50ms runtime).
