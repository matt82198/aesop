...
- `self_stats.py` — Git-derived metrics counter + README block generator. `extract_model_tier(model_name)` parses and normalizes model names: removes context hints in parens, deduplicates variants (Opus 5.0 → Opus 5, Haiku 4 → Haiku 4.5) to canonical tier form (6 tiers collapsed to 5: Haiku 4.5, Sonnet 4, Opus 4.8, Opus 5, Fable 5). Used by stats.json generation to classify cost/quality data by model tier rather than full version string.
