...
- `ci_workflow_lint.py` — CI workflow linter (YAML parsing, npm ci lockfile checks, test coverage, CLI gate parity); Guardrail: verifies all documented verify_*.py CI gates are actually wired into .github/workflows/ci.yml (catches escape: gate added to pre-push hook but omitted from workflow)
...
- `verify_test_suite_count.py` — Test suite count drift gate (auto-verifiable + auto-fixable); CLI: `--check` (fail if counts drift; CI gate) | `--fix [--dry-run]` (auto-rewrite tests/CLAUDE.md counts to match disk; lanes use this); --repo/--claudemd overrides; idempotent; exit 0=clean/fixed, 1=drift/error (staged for CI wiring in .github/workflows/ci.yml via in-flight PR)
