...
- **Subprocess encoding (G10)**: every `subprocess.run`/`Popen` decoding output passes explicit `encoding='utf-8'` with `errors='replace'` handler. The platform default is cp1252 on Windows and corrupts non-ASCII output; without an errors handler, Python crashes on non-UTF-8 bytes (e.g., byte 0x97 em-dash). `encoding_lint.py` scans the WHOLE repo and enforces both requirements, so one violation anywhere blocks every Python-touching push. Existing backlog violations are tracked in `.encoding-baseline.json`.
