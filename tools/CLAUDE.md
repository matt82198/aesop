...
- `ci_shard_runner.py` — Shard-aware Python test runner (distributes tracked test files across N shards; timing-aware greedy bin-packing via `--timing-file PATH`, round-robin fallback; `--emit-timing` writes per-module wall-clock JSON; PYTEST_TIMEOUT env var switches to pytest subprocess with per-test timeout via pytest-timeout; spawn-safe with __main__ guard; used by ci and windows-shard jobs)
