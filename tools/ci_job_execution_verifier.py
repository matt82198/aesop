...
INDEX: Silent-CI-job detector: parses one workflow's job/step inventory (name/if/needs/matrix) and accounts it against real run history (`gh run list` + `gh api .../runs/<id>/jobs`, bounded to `--runs` sampled runs) to catch jobs and named steps DEFINED but never EXECUTED (renamed steps, `if:`-false gates, dead `needs:` chains) — the complement to `verify_gates_wired.py` (G2.6), which asks whether a documented gate is present in the YAML; this asks whether the YAML job actually runs. Job-name matching is anchored + matrix-suffix aware so `windows` never absorbs `windows-shard (0)`, and `${{ }}` in a name matches any rendering; step evidence is only counted from green jobs (a failed job truncates its step list). CLI: `[--workflow ci.yml] [--lookback-days 14] [--runs 5] [--root DIR] [--json] [--fixture PATH]` (`--fixture` = offline payloads, hermetic test seam); exit 0=clean / 1=findings / 2=gh-or-parse failure OR zero runs in lookback (fail-closed: no evidence is never a green). Needs PyYAML + gh; NOT yet wired into ci.yml
