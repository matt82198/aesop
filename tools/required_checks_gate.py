#!/usr/bin/env python3
"""
INDEX: Required-checks drift gate; the REVERSE of `ci_gate_runability.py`. That tool asks "can this required check run?" (reads workflows); this asks "is this gating job actually required?" — a ci.yml job intended to gate merges but absent from branch protection's required set runs, goes red, and the PR merges anyway, and nothing in the repo records that it was supposed to gate. `tools/required-checks.json` is that missing record (declared intent + a dated snapshot of the live API response); the gate diffs it against live protection and fails on drift in EITHER direction: `MISSING` (declared but unenforced = never blocks) / `UNDECLARED` (enforced but unreviewed). Reads `required_status_checks.contexts`, falling back to `checks[].context` (GitHub is deprecating the flat list); a bare-`message` payload (404 "Branch not protected", 403, rate limit) is an API error, not an empty required set. Intentionally does NOT use `subprocess_common.gh()` — that helper is reached by `sys.path.insert` on `tools/`, which the fail-closed `import_resolution_check.py` pre-push gate cannot resolve (its module map indexes only top-level modules and `__init__.py` packages); existing callers pass only because that gate scans staged files. CLI: `[--intent PATH] [--protection-file PATH] [--repo OWNER/NAME] [--branch B] [--json]`; exit 0=match/1=drift/2=error (gh missing or unauthenticated, malformed JSON, empty `required_contexts`). `--protection-file` diffs a captured payload offline — used by the hermetic tests and to review a protection change before applying it. Current declared set: `ci (0..3)` + `windows`; stdlib-only
